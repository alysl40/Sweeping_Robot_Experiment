...
class Line:                                                      # The class Line code is written by myself
    ...
    def Draw_a_left_top_line(self,canvas):
        canvas.create_line(self.x,self.y,1000,0,fill = "black",width = 5)
        
    def Draw_a_left_button_line(self,canvas):
        canvas.create_line(self.x,self.y,1000,1000,fill = "black",width = 5)
...
def register(canvas):
    ...
    noOfBots = 4
    noOfDirt = 500
    line1 = Line(0,1000)                                   #The following code is written by myself
    registryPassives.append(line1)
    line1.Draw_a_left_top_line(canvas)
    line2 = Line(0,0)
    registryPassives.append(line2)
    line2.Draw_a_left_button_line(canvas)
    for i in range(0,noOfBots):
        if i ==0:
            bot = Bot("Bot"+str(i),canvas,500,750)
            bot_list.append(bot)
            bot.draw(canvas)
        elif i==1:
            bot = Bot("Bot"+str(i),canvas,500,250)
            bot_list.append(bot)
            bot.draw(canvas)
        elif i==2:
            bot = Bot("Bot"+str(i),canvas,250,500)
            bot_list.append(bot)
            bot.draw(canvas)
        elif i==3:
            bot = Bot("Bot"+str(i),canvas,750,490)
            bot_list.append(bot)
            bot.draw(canvas)
...
def moveIt(canvas,bot_list,registryPassives,count,moves,window):
    ...
    for rr in bot_list:                                                            #The following code is written by myself
        if rr.name == "Bot0":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,1000-rr.y,rr.y,500,1000)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        elif rr.name == "Bot1":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,rr.y,1000-rr.y,0,500)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        elif rr.name == "Bot2":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,0,500,rr.x,1000-rr.x)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        elif rr.name == "Bot3":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,500,1000,1000-rr.x,rr.x)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        print(moves)
        numberOfMoves = 1000
