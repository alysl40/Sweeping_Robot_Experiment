...
class Line:                                                # The class Line code is written by myself
    ...
    def Draw_a_horizontal_line(self,canvas):
        canvas.create_line(self.x,self.y,1000,500,fill = "black",width = 5)
        
    def Draw_a_vertical_line(self,canvas):
        canvas.create_line(self.x,self.y,500,0,fill = "black",width = 5)
...
def register(canvas):
    ...
    noOfBots = 4
    noOfDirt = 500
    line1 = Line(0,500)                                                  #The following code is written by myself
    registryPassives.append(line1)
    line1.Draw_a_horizontal_line(canvas)
    line2 = Line(500,1000)
    registryPassives.append(line2)
    line2.Draw_a_vertical_line(canvas)
    for i in range(0,noOfBots):                            
        if i ==0:
            bot = Bot("Bot"+str(i),canvas,250,750)
            bot_list.append(bot)
            bot.draw(canvas)
        elif i==1:
            bot = Bot("Bot"+str(i),canvas,750,250)
            bot_list.append(bot)
            bot.draw(canvas)
        elif i==2:
            bot = Bot("Bot"+str(i),canvas,250,250)
            bot_list.append(bot)
            bot.draw(canvas)
        elif i==3:
            bot = Bot("Bot"+str(i),canvas,750,750)
            bot_list.append(bot)
            bot.draw(canvas)
...
def moveIt(canvas,bot_list,registryPassives,count,moves,window):
    ...
    for rr in bot_list:                                                                  #The following code is written by myself
        if rr.name == "Bot0":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,0,500,500,1000)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        elif rr.name == "Bot1":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,500,1000,0,500)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        elif rr.name == "Bot2":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,0,500,0,500)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        elif rr.name == "Bot3":
            rr.brain()
            rr.move(canvas,registryPassives,1.0,500,1000,500,1000)
            registryPassives = rr.collectDirt(canvas,registryPassives, count)
        print(moves)
        numberOfMoves = 1000
